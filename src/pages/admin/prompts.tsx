...
import { Plus, Trash2, MessageSquare, Check, AlertCircle, X } from 'lucide-react';

/**
 * Admin Prompts Page - Prompt Management
 * 
...
export default function AdminPromptsPage({ user }: AdminPromptsProps) {
  ...
  return (
    <Layout title="Admin Panel - Prompt Management" description="Manage AI prompts">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        <div className="mb-8">
          <h1 className="text-3xl font-bold">Prompt Management</h1>
          <p className="text-text-secondary mt-2">
            Welcome, {user.name}. Create and manage AI prompts for your users.
          </p>
        </div>
        ...
        {/* Create Prompt Form */}
        <div className="bg-bg-secondary rounded-2xl border border-border-primary p-6 mb-8 shadow-soft">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
                ...
                <input
                  ...
                  className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 ${
                    validationErrors.title 
                      ...
                      : 'focus:ring-brand-primary/20 border-border-primary'
                  }`}
              ...
              <div>
                ...
                {validationErrors.category && (
                  <p className="mt-1 text-sm text-status-error">{validationErrors.category}</p>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 border border-border-primary rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary/20"
            ...
            <div>
              ...
              />
              {validationErrors.prompt && (
                <p className="mt-1 text-sm text-status-error">{validationErrors.prompt}</p>
              )}
            </div>
            ...
            <button
              ...
              className={`w-full py-3 px-6 rounded-lg font-semibold transition flex items-center justify-center gap-2 ${
                submitting
                  ...
                  : 'bg-brand-primary text-brand-secondary hover:bg-interactive-hover'
              }`}
            >
              {submitting ? (
                <>
        ...
        {/* Existing Prompts */}
        <div className="bg-bg-secondary rounded-2xl border border-border-primary p-6 shadow-soft">
          <h2 className="text-xl font-semibold mb-4">Existing Prompts ({prompts.length})</h2>
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm text-text-muted mt-1">Create your first prompt using the form above.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {prompts.map((prompt) => (
                <div
                  key={prompt._id}
                  className="border border-border-primary rounded-xl p-5 hover:shadow-brand transition-all bg-bg-primary"
                >
                      ...
                      <div className="flex items-start justify-between gap-4 mb-2">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <h3 className="font-display font-semibold text-lg text-text-primary">{prompt.title}</h3>
                            <span className="text-xs bg-brand-primary/10 text-brand-primary px-2 py-1 rounded-full font-semibold">
                              {prompt.category}
                            </span>
                          </div>
                          ...
                          {prompt.tags.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-2">
                              {prompt.tags.map((tag) => (
                                <span key={tag} className="text-xs bg-text-muted/10 text-text-muted px-2 py-0.5 rounded">
                                  #{tag}
                                </span>
                              ))}
                            </div>
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="px-3 py-1 bg-status-error text-brand-secondary text-xs rounded hover:bg-red-700 transition flex items-center gap-1"
                          >
      ...
      {deleteConfirmId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm">
          <div className="relative bg-bg-primary rounded-xl shadow-2xl max-w-md w-full border border-border-primary">
            <div className="p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h2 className="text-xl font-bold text-text-primary">Delete Prompt</h2>
              ...
              <div className="flex gap-3 justify-end">
                <button
                  ...
                  className="px-4 py-2 text-text-secondary bg-bg-secondary rounded-lg hover:bg-bg-tertiary transition"
                >
                  Cancel
                </button>
                <button
                  onClick={() => handleDelete(deleteConfirmId)}
                  className="px-4 py-2 bg-status-error text-brand-secondary rounded-lg hover:bg-red-700 transition"
                >
                  Delete Prompt
                </button>
              </div>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  try {
    ...
    console.error('Error in admin prompts page:', error);
